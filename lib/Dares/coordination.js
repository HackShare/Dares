...
Coordination.prototype.write = function ( key, value, useResult ) {
    ...
    if ( this.state.name !== 'idle' ) {
        useResult( {
            error: 'Cannot start write because there is currently an operation in progress: ' + this.state.name + ' ' + this.state.key
        } );
    } else {
        //set the state
        this.state = {
...
Coordination.prototype.read = function ( key, useResult ) {
    ...
    if ( this.state.name !== 'idle' ) {
        useResult( {
            error: 'Cannot start read because there is currently an operation in progress: ' + this.state.name + ' ' + this.state.key
        } );
    } else {
        //set the state
        this.state = {
