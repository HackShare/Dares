...
var Process = function ( Dares, setupComplete ) {
    ...
    if ( options.logSettings ) {
        var transports = [];
        for ( var transport in options.logSettings ) {
            if ( options.logSettings.hasOwnProperty( transport )) {
                if ( transport === 'console' ) {
                    transports.push( new ( winston.transports.Console )({
                        level: options.logSettings[transport]
                    }) );
                } else {
    ...
    this.stop = function ( callback ) {
        ...
        this.logger.info( 'Process ' + this.id + ' is shutting down.' );
    };
