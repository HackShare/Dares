...
ReadCoordination.prototype._lock = function () {
    assert( this.coordination.state.name === 'beginRead',
        'Wrong state, expected "beginRead" but got ' + this.coordination.state.name );

    //updating the state
    this.coordination.state = {name: 'waitForLocks',
...
ReadCoordination.prototype._processRead = function ( key, value, version ) {
    assert( key === this.coordination.state.key,
