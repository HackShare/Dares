...
var Reactions = function ( p ) {
    ...
    reactions.notAdded = function () {
        thatCoordinator.gotAnEpochChange( {
    ...
    reactions.allLocked = function ( input ) {
        if ( thatCoordinator.state.name === 'waitForAllLocks' ) {
            var data = input.data;
            thatCoordinator.epochChangeCoordination._processAllLock( true, data );
        }
    };
    ...
    reactions.nothingLocked = function ( input ) {
        if ( thatCoordinator.state.name === 'waitForAllLocks' ) {
            var data = input.data;
            thatCoordinator.epochChangeCoordination._processAllLock( false, data );
        }
    };
    ...
    reactions.plainRead = function ( input, address ) {
        ...
        send( {
            action: 'plainReadValue',
            data: {
                key: key,
                value: data.value,
                version: typeof data.version !== 'undefined' ? data.version : -1
            },
            port: owningProcess.port
        }, address, input.port );
    };
    ...
    reactions.plainReadValue = function ( input ) {
        var data = input.data;
        thatCoordinator.epochChangeCoordination._processPlainRead( data.key, data.value, data.version );
    };
    ...
    reactions.epochChangeACK = function () {
        thatCoordinator.epochChangeCoordination._epochAcknowledged();
    };
    ...
    reactions.voteYes = function ( input ) {
        if ( thatCoordinator.state.name === 'waitForVotes' ) {
            var data = input.data;
            thatCoordinator.writeCoordination._processVote( true, data );
        }
    };
    ...
    reactions.voteNo = function ( input ) {
        if ( thatCoordinator.state.name === 'waitForVotes' ) {
            var data = input.data;
            thatCoordinator.writeCoordination._processVote( false, data );
        }
    };
    ...
    reactions.processACK = function () {
        if ( thatCoordinator.state.name === 'waitForACK' ) {
            thatCoordinator.writeCoordination._voteForWrite();
        }
    };
    ...
    reactions.commit = function ( input, address ) {
        var key = input.data;
        if ( reactions.committing[key] ) {
            reactions.committing[key].func();
        }
        send( {
            action: 'haveCommitted',
            data: key,
            port: owningProcess.port
        }, address, input.port );
    };

    reactions.haveCommitted = function () {
        thatCoordinator.writeCoordination.processCommit();
    };
    ...
    reactions.readLocked = function ( input ) {
        if ( thatCoordinator.state.name === 'waitForLocks' ) {
            var data = input.data;
            thatCoordinator.readCoordination._processReadLock( true, data );
        }
    };
    ...
    reactions.readNotLocked = function ( input ) {
        if ( thatCoordinator.state.name === 'waitForLocks' ) {
            var data = input.data;
            thatCoordinator.readCoordination._processReadLock( false, data );
        }
    };
    ...
    reactions.readValue = function ( input ) {
        var data = input.data;
        thatCoordinator.readCoordination._processRead( data.key, data.value, data.version );
    };
};
